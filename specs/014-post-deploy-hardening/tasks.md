...
- [ ] T010 [P] [US2] Add `"e2e:playwright-smoke": "playwright test --grep @smoke"` to `neko-hoa/package.json`; keep `e2e` (full suite) and `e2e:playwright-dev` available for local/PR use (FR-007).
...
- Final production threshold values (auth/payments/unknown per minute) are tunable config; confirm in T007/T024.
