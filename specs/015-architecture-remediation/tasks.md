...
- [X] T020 [P] [US3] Startup-validation tests in `HOAManagementCompany.Tests/Integration/Configuration/ProductionBackstopValidationTests.cs`: Production + `Startup:SeedData=true` → fail-fast; Production + `DevTools:E2ECleanupEnabled=true` → fail-fast; Dev/Test environments with same flags → boot succeeds; PLUS unset/unknown `ASPNETCORE_ENVIRONMENT` with test-support flags set → boot refused (spec Edge Case: ambiguous environment identity defaults to "test machinery disabled"; pins the existing 008 env-name validation as the backstop) (Theory over environments; red today)
- [X] T021 [P] [US3] Endpoint backstop test delivered as container-free unit tests in `HOAManagementCompany.UnitTests/DevTools/E2ECleanupBackstopTests.cs` (FastEndpoints test factory; a full-Production in-process boot with the flag set is impossible by design — startup validation refuses it, which T020 proves): host environment Production with flag set → 404 and a security-relevant Serilog event recorded; Dev with flag set → still works (defense-in-depth path, distinct from T020's boot rejection)

### Implementation for User Story 3

- [X] T022 [US3] Extend the startup options validation in `HOAManagementCompany/Infrastructure/Configuration/` (e.g. `HostEnvironmentValidator`/startup options validator per research R8) with environment-aware rules rejecting `Startup:SeedData` and `DevTools:E2ECleanupEnabled` in `Production`; T020 green
- [X] T023 [US3] Add `IHostEnvironment.IsProduction()` guard to `HOAManagementCompany/Features/DevTools/E2ECleanupEndpoint.cs` returning 404 and logging a Serilog security event when invoked with the flag set in Production (keep config-flag gate for non-prod; update the explanatory comment); T021 green
- [X] T024 [P] [US3] Change `HOAManagementCompany/HOAManagementCompany.csproj:65` to `<Content Include="testdata\**\*" CopyToOutputDirectory="PreserveNewest" CopyToPublishDirectory="Never" />` and verify `dotnet publish -c Release` output contains no `testdata/` (embedded fallback in `Infrastructure/Storage/TestDataFiles.cs` already covers runtime)
...
- [X] T036 [P] [US5] Extract the duplicated scheduler-secret check (`Features/Payments/Jobs/RunDraftsEndpoint.cs:35-43` ≡ `ReconcileEndpoint.cs:28-36`) into a shared helper/pre-processor in `HOAManagementCompany/Features/Payments/Jobs/SchedulerAuth.cs` (constant-time compare preserved)
- [X] T037 [P] [US5] Delete dead artifacts: root `/Dockerfile` (nothing builds it) and `HOAManagementCompany/Components/` (misfiled `.code-workspace` moved to repo root or deleted); verify `docker-compose.yaml` and workflows still reference `HOAManagementCompany/Dockerfile` only (FR-016)
...
- [X] T038 [US6] Move `HOAManagementCompany.Tests/Unit/**` (88 facts: FeeCalculator, AllocationService, ReceiptFactory, PaymentConfigService, 9 validator suites) into `HOAManagementCompany.UnitTests/`, preserving namespaces per folder convention; remove the now-empty `Unit/` tree; both projects green (FR-017)
...
- [X] T041 [P] [US6] Extract composite actions `.github/actions/dotnet-setup/`, `.github/actions/node-setup/`, `.github/actions/gcloud-auth/` and adopt them across the 9 workflows; make `infra-plan.yml`/`infra-apply.yml` use the existing `pr-env-tofu-init` composite and delete the dead commented block at `infra-apply.yml:87-93`
- [X] T042 [US6] Extract `infra/modules/cloud-run-service/` shared core — DELIVERED for the Cloud Run service + public-invoker surface (the ~150-line near-twin), consumed by both modules as thin wrappers WITH `moved` blocks so live state never destroy/recreates; the secret-ref wiring dedupes via the `secret_env_refs` map. Secrets/Neon resources stay per-module deliberately: their logic is genuinely variant (per-PR placeholder + forked-branch flow vs. primary DB + operator seeding), not copy-paste. All 34 .tf files HCL-parse locally; authoritative `tofu fmt/validate/plan` runs in the `infra-plan` CI job on this PR (no tofu binary/provider registry available in the implementation sandbox) (FR-019)
...
- [X] T044 [US6] Decompose `neko-hoa/src/app/features/payments/one-time/one-time.component.ts` (341 lines, 41 inline styles): extract `StripeElementsHostComponent` (shared, `neko-hoa/src/app/shared/stripe-elements-host/`), a signal-based wizard store, and split template/styles into `.html`/`.scss`; replace the 12 hand-rolled error sites with the central mechanism; component tests via Angular Testing Library; Cypress payments E2E stays green
- [X] T045 [US6] Decompose `neko-hoa/src/app/features/payments/recurring/recurring.component.ts` (329 lines, 25 inline styles, 17 error sites) the same way, reusing `StripeElementsHostComponent` and the shared error presentation; component tests; Cypress green
...
- [X] T046 [P] Update `docs/architecture-smells-analysis.md` with a remediation-status column mapping each finding to its landed fix (report stays truthful)
- [X] T047 [P] Verify Sentry capture after the error-mapping change — code-level verification: DomainException path logs Information with code/status/path (no Sentry noise for expected business errors); the unhandled-exception path (LogError + Sentry) is unchanged; consistency detection emits SentrySdk.CaptureMessage(Warning) with ids only, no amounts. Live-trace verification happens on the Dev deploy (release.yml)
- [X] T048 [P] Accessibility pass on decomposed payment screens and the shared error component: keyboard access, labels, validation messages (WCAG 2.1 AA behavior preserved)
- [X] T049 Review new/changed tests for Theory usage, parallel safety, rerun safety, unique data, and factory discipline (testing constitution §2.1/2.3/2.5)
- [X] T050 Sonar PR scan + Codecov + 90% diff-cover gates run in ci.yml on this PR (the slices shipped as one PR train on branch 015-architecture-remediation); hotspot-class code (`PaymentRecorder`, `GlobalExceptionHandler`, `ClaimsPrincipalExtensions`, backstop validators, `StripeEventTranslator`, wizard stores) all carries dedicated tests
- [X] T051 Repowise: regenerate/confirm marker regions in the six files listed in plan.md (`PaymentRecorder.cs`, `WebhookProcessor.cs`, `DomainException.cs`, `GlobalExceptionHandler.cs`, `LayeringTests.cs`, and `neko-hoa/src/app/core/api/README.md` — the companion doc; `generated-types.ts` itself stays marker-free because it is byte-exact codegen output guarded by the drift gate) so indexed docs match merged code
- [X] T052 **Before each slice's PR**: bring this feature's `spec.md` AND `tasks.md` up to date with work actually performed; update any older `spec.md` that drifted (spec 006 payments flows must still read truthfully after the `PaymentRecorder`/neutral-event refactor — reconcile and record which spec prevails if any contradiction emerges)
- [X] T053 Verify the spec stays executable: every mandatory acceptance scenario (US1–US6) is traceable to a currently-passing automated test (T004–T007, T014–T015, T020–T021, drift gate, T031–T032, tier/pipeline checks); no unverified spec claims
- [X] T054 quickstart.md validation — run locally where the sandbox allows: container-free tier (33 tests, <1 s run / well under 60 s incl. build), backstop boot checks (6 green), publish testdata exclusion (verified empty), codegen determinism (byte-identical re-run), HCL parse of all infra files; Docker-dependent integration tiers and tofu validate execute in CI (ci.yml + infra-plan) — no Docker/tofu in the implementation sandbox
