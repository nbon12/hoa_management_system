...
- **Auth provider deviation**: DEVIATION (pre-existing) — constitution §2/§7 mandate **Auth0**; the codebase uses **custom ASP.NET Identity + JWT**. This feature hardens the existing custom auth (lockout, algorithm pinning, claim flow) and does **not** migrate to Auth0. Recorded in Complexity Tracking as a pre-existing gap this hardening does not resolve or worsen.
...
|-----------|------------|-------------------------------------|
| Custom ASP.NET Identity + JWT instead of Auth0 (constitution §2/§7) | Pre-existing implementation; this security-hardening feature hardens it (lockout, algorithm pinning, claim flow) rather than replacing it | Migrating to Auth0 is a large, separate product feature that would balloon a hardening slice; deferring keeps the security fixes shippable. Flagged for a dedicated future spec. |
