...
- **FR-019**: The .NET backend MUST emit logs in structured JSON format so that the local dashboard and any downstream vendor can parse and ingest them without custom transformation.
- **FR-020**: Serilog MUST be used as the structured logging library in the .NET backend and MUST be configured to output logs in structured JSON format to the configured OTLP destination endpoint.
