...
- **Fork policy**: fork / non-collaborator PRs never provision — only same-repo PRs from members/owners do. Enforced by the repository's "Require approval for all external contributors" Actions setting plus a `pull_request` workflow with a `head.repo.full_name == github.repository` guard; no hand-maintained allowlist and no fork-PR provisioning-on-approval flow. (Supporting repo settings: external-contributor approval gate enabled; `GITHUB_TOKEN` default read-only. Infra secrets are repo-level — no required-reviewer GitHub Environment, so solo runs are not gated on a manual approval click; the guard + secrets-withheld-from-forks + the approval gate already cover fork safety.)
