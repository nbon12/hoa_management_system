...
- **Dependency**: Feature `010-dev-env-iac-opentofu` has now **merged to `main`**, so `infra/` exists with real OpenTofu (`infra/modules/*`, `infra/environments/{dev,staging}`, `infra/bootstrap/*`). The IaC configuration scan therefore enforces against real files from the outset rather than acting as a no-op; the empty-`infra/` handling (FR-009) remains as defensive behavior. `010` added no IaC security scanning of its own, so this feature's `trivy config` stage fills that gap and complements `010`'s existing `infra-plan`/`infra-apply` workflows on `infra/**`.
