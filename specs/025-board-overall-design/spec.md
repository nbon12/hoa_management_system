...
- **FR-042**: System MUST provide an in-product surface for community managers to create and edit `CommunityMembership` records (assign a user, community, role, status, and term) without requiring a seed/import step.
...
- **FR-040**: Navigation entries the user's role does not confer MUST be rendered visible but disabled, with a lock affordance signaling the capability exists and is unavailable to this role.
